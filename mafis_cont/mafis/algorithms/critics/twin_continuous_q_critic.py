...
import torch
from mafis.models.value_function_models.continuous_q_net import ContinuousQNet
from mafis.utils.envs_tools import check
from mafis.utils.models_tools import update_linear_schedule
...
class ScoreFunction(nn.Module):
    def __init__(self, obs_shape, action_shape):
        ...
        self.time_shape = 64
        self.time_emb = SinusoidalEmbedding(self.time_shape)
        self.fc1 = nn.Linear(obs_shape + action_shape + self.time_shape, 128)
        self.fc2 = nn.Linear(128, 128)
        self.fc3 = nn.Linear(128, self.action_shape)
        self.relu = nn.ReLU()

        
    def forward(self, t, obs, action):
        t_emb = self.time_emb(t)
        x = torch.cat([t_emb, obs, action], dim=-1)
...
class TwinContinuousQCritic:
    ...
    def __init__(
        ...
        self.critic = ContinuousQNet(args, obs_space, act_space, device)
        self.critic2 = ContinuousQNet(args, obs_space, act_space, device)
        ...
        critic_params = itertools.chain(
            self.critic.parameters(), self.critic2.parameters(), self.mixer.parameters()
        )
    ...
    def save(self, save_dir):
        ...
        torch.save(self.mixer.state_dict(), str(save_dir) + "/mixer" + ".pt")

    def restore(self, model_dir):
        """Restore the model parameters."""
        print("Restoring the critic model from:", str(model_dir) + "/critic_agent" + ".pt")
        ...
        mixer_state_dict = torch.load(str(model_dir) + "/mixer" + ".pt")
        self.mixer.load_state_dict(mixer_state_dict)
