...
import numpy as np
import torch
from mafis.envs.env_wrappers import ShareSubprocVecEnv, ShareDummyVecEnv
...
def make_train_env(env_name, seed, n_threads, env_args):
    ...
    if env_name == "dexhands":
        from mafis.envs.dexhands.dexhands_env import DexHandsEnv
    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                from mafis.envs.smac.StarCraft2_Env import StarCraft2Env

                env = StarCraft2Env(env_args)
            elif env_name == "smacv2":
                from mafis.envs.smacv2.smacv2_env import SMACv2Env

                env = SMACv2Env(env_args)
            elif env_name == "mamujoco":
                from mafis.envs.mamujoco.multiagent_mujoco.mujoco_multi import (
                    MujocoMulti,
                )

                env = MujocoMulti(env_args=env_args)
            elif env_name == "pettingzoo_mpe":
                from mafis.envs.pettingzoo_mpe.pettingzoo_mpe_env import (
                    PettingZooMPEEnv,
                )
                ...
                from mafis.envs.gym.gym_env import GYMEnv

                env = GYMEnv(env_args)
            elif env_name == "football":
                from mafis.envs.football.football_env import FootballEnv

                env = FootballEnv(env_args)
            elif env_name == "lag":
                from mafis.envs.lag.lag_env import LAGEnv
...
def make_render_env(env_name, seed, env_args):
    ...
    if env_name == "smac":
        ...
        from mafis.envs.mamujoco.multiagent_mujoco.mujoco_multi import MujocoMulti

        env = MujocoMulti(env_args=env_args)
        env.seed(seed * 60000)
    elif env_name == "pettingzoo_mpe":
        from mafis.envs.pettingzoo_mpe.pettingzoo_mpe_env import PettingZooMPEEnv
...
def get_num_agents(env, env_args, envs):
    ...
    if env == "smac":
        from mafis.envs.smac.smac_maps import get_map_params
